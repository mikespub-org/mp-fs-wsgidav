...
def verify_user_session(session):
    ...
    if not auth_user:
        ...
        if AuthorizedUser.get_count() < 1:
            logging.debug(
                "Assign admin rights to first AuthorizedUser(%s)" % auth_user.email
            )
    ...
    if session.claims and not auth_user.claims:
        ...
        if (
            "firebase" in session.claims
            and "sign_in_provider" in session.claims["firebase"]
        ):
            auth_user.auth_domain = (
                "Firebase(%s)" % session.claims["firebase"]["sign_in_provider"]
            )
        else:
