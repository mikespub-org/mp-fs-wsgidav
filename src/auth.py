    ...
    def decorator(f):
        ...
        def decorated_function(*args, **kwargs):
            ...
            if not session.is_user() and access in ('browser', 'reader', 'editor'):
                output = "You need to login as user <a href='%s'>Login</a>" % sessions.LOGIN_URL
                return output
                #return redirect(users.create_login_url(request.url))
            if access in ('reader', 'editor'):
                if access == 'editor' and 'editor' not in session.get_roles():
                    output = "You need to login as editor <a href='%s'>Logout</a>" % sessions.LOGOUT_URL
                    return output
            return f(*args, **kwargs)
...
def user_token():
    ...
    return render_template(
        'auth_token.html',
        user_claims=session.claims, error_message=error_message,
        auth_url=sessions.AUTH_URL, logout_url=sessions.LOGOUT_URL,
